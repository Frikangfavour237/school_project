...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($stmt->num_rows > 0) {
        ...
        // Insert the like action into the feedback_likes table
        $stmt = $conn->prepare("INSERT INTO feedback_likes (feedback_id, student_id, action) VALUES (?, ?, 'like')");
        $stmt->bind_param("ii", $feedback_id, $student_id);
        $stmt->execute();

        // Update the likes count in the feedback table
        $stmt = $conn->prepare("UPDATE feedback SET likes = likes + 1 WHERE id = ?");
