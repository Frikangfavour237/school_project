...
### Step 6: Create `disapprove_attendance.php` Script

Create a script to handle the disapproval of attendance requests.
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Update the status of the attendance request
    $stmt = $conn->prepare("UPDATE attendance_requests SET status = 'disapproved' WHERE id = ?");
    $stmt->bind_param("i", $request_id);
